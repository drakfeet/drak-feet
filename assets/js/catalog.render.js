...
const CatalogRender = {
  ...
  renderizarMenuCategorias(produtosPorCategoria) {
    const menuMobile = document.getElementById('menuCategorias');
    ...
    const htmlFinal = btnTodas + categoriasHtml;

    // Renderizar no menu desktop
    if (menuDesktop) {
      const scrollDiv = menuDesktop.querySelector('.categorias-scroll');
      scrollDiv.innerHTML = htmlFinal;
    ...
    // Renderizar no menu mobile (se existir)
    if (menuMobile) {
      const scrollDiv = menuMobile.querySelector('.categorias-scroll');
      if (scrollDiv) {
        scrollDiv.innerHTML = htmlFinal;
  ...
  criarCardProduto(produto) {
    const descontoPix = this.config.descontoPix || 10;
    const parcelasSemJuros = this.config.parcelasSemJuros || 3;
    const parcelasTexto = parcelasSemJuros > 1 ? `até ${parcelasSemJuros}x sem juros` : '';
        ...
        <div class="produto-info">
          ...
          <div class="produto-precos">
            ...
            <div class="preco-item destaque">
              <span class="desconto-badge">${descontoPix}% OFF</span>
