...
const CatalogRender = {
  ...
  renderizarMenuTipos() {
    ...
    const mobileMenuContent = document.querySelector('#mobileMenu .mobile-menu-content');
    const targetParent = isMobile && mobileMenuContent ? mobileMenuContent : header;
    ...
    if (!menuTipos) {
      ...
      targetParent.appendChild(menuContainer);
      menuTipos = menuContainer;
    }

    if (menuTipos.parentElement !== targetParent) {
      if (isMobile) {
        targetParent.insertBefore(menuTipos, targetParent.firstChild);
      } else {
        targetParent.appendChild(menuTipos);
      }
    }

    menuTipos.classList.toggle('menu-tipos-mobile', isMobile);
  ...
  renderizarMenuMobile(produtos) {
    ...
    const menuLinks = (this.config?.menuLinks || []).filter(link => {
      if (link.ativo === false) return false;
      const texto = String(link.texto || '').toLowerCase().trim();
      return texto !== 'filtro' && texto !== 'filtros';
    });
  ...
  criarCardProduto(produto) {
    ...
    const entregaSelecionada = window.CartService && CartService.getDeliveryLabel
      ...
      : '';
    const inWishlist = window.WishlistService && WishlistService.has(produto.id);
      ...
      <div class="produto-card" data-produto-id="${produto.id}">
        ...
        <div class="produto-info">
          ...
          <div class="produto-actions">
            <button class="btn-wishlist ${inWishlist ? 'active' : ''}" data-action="toggle-wishlist" data-produto-id="${produto.id}">
              <span class="wishlist-heart" aria-hidden="true">${inWishlist ? '♥' : '♡'}</span>
              <span>${inWishlist ? 'Remover da Wishlist' : 'Adicionar à Wishlist'}</span>
            </button>
            <button class="btn-add-cart" data-action="add-cart" data-produto-id="${produto.id}">
              Adicionar ao Carrinho
            </button>
            <button class="btn-whatsapp" data-action="buy-whatsapp" data-produto-id="${produto.id}">
              Comprar no WhatsApp
            </button>
          </div>
        </div>
  ...
  configurarEventListeners() {
    const produtosContainer = document.getElementById('produtosPorCategoria');
    if (produtosContainer && !this._productActionEventsBound) {
      produtosContainer.addEventListener('click', (event) => {
        const button = event.target.closest('button[data-action][data-produto-id]');
        if (!button) return;
        ...
        if (button.dataset.action === 'toggle-wishlist') {
          this.toggleWishlist(produtoId);
          return;
        }
        if (button.dataset.action === 'buy-whatsapp') {
          this.enviarWhatsApp(produtoId);
        }
      });
      this._productActionEventsBound = true;
    }
  ...
  toggleWishlist(produtoId) {
    ...
    const produto = this.produtosTodos.find(p => p.id === produtoId);
    if (!produto) return;
    WishlistService.toggleItem(produto);
    this.renderizarPorCategorias(this.produtosTodos);
  },
  ...
  aplicarCustomizacao(config) {
    ...
    if (custom.corSecundaria) root.style.setProperty('--whatsapp', custom.corSecundaria);
    if (custom.corWhatsappFlutuanteBg) root.style.setProperty('--whatsapp', custom.corWhatsappFlutuanteBg);

    const variableMap = {
      corBarraAvisoBg: '--aviso-bar-bg',
      corBarraAvisoTexto: '--aviso-bar-text',
      corFiltroBotaoBg: '--btn-filtros-bg',
      corFiltroBotaoTexto: '--btn-filtros-text',
      corFiltrosPainelBg: '--filtros-bg',
      corFiltrosPainelBorda: '--filtros-border',
      corFiltroItemHover: '--filtros-item-hover',
      corMenuTiposBg: '--menu-tipos-bg',
      corMenuTiposBotaoBg: '--menu-tipos-btn-bg',
      corMenuTiposBotaoTexto: '--menu-tipos-btn-text',
      corMenuTiposBotaoAtivoBg: '--menu-tipos-btn-active-bg',
      corMenuTiposBotaoAtivoTexto: '--menu-tipos-btn-active-text',
      corEntregaSelectorBg: '--entrega-selector-bg',
      corEntregaSelectorBorda: '--entrega-selector-border',
      corEntregaOpcaoBg: '--entrega-option-bg',
      corEntregaOpcaoTexto: '--entrega-option-text',
      corEntregaOpcaoAtivaBg: '--entrega-option-active-bg',
      corEntregaOpcaoAtivaTexto: '--entrega-option-active-text',
      corWhatsappTooltipBg: '--whatsapp-tooltip-bg',
      corWhatsappTooltipTexto: '--whatsapp-tooltip-text',
      corSliderBotaoBg: '--slider-btn-bg',
      corSliderBotaoTexto: '--slider-btn-text',
      corSliderDotBg: '--slider-dot-bg',
      corSliderDotAtivo: '--slider-dot-active-bg',
      corCarrinhoBg: '--cart-bg',
      corCarrinhoBorda: '--cart-border',
      corCarrinhoHeaderBg: '--cart-header-bg',
      corCarrinhoFooterBg: '--cart-footer-bg',
      corCarrinhoItemBg: '--cart-item-bg',
      corCarrinhoTotalTexto: '--cart-total-text'
    };

    Object.entries(variableMap).forEach(([customKey, cssVar]) => {
      if (custom[customKey]) {
        root.style.setProperty(cssVar, custom[customKey]);
      }
    });

    // Garante que o tema claro/escuro continue aplicando a paleta completa
    // mesmo após customizações de loja.
    if (window.ThemeService && typeof ThemeService.getCurrentTheme === 'function') {
