...
const TrackingService = {
  ...
  initFacebookPixel(pixelId) {
    try {
      ...
      const existing = document.querySelector('script[data-fb-pixel="true"]');
      if (existing) return;
      ...
      script.onload = () => {
        try {
          ...
          console.warn('Facebook Pixel init failed after load:', error);
        }
      };
      script.onerror = () => {
        this.facebookInitialized = false;
        console.warn('Facebook Pixel blocked or failed to load (adblock/privacy extension).');
      };
