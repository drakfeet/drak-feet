...
const DataService = {
  ...
  async buscarConfig() {
    try {
      ...
      const [banners, menuLinks, redesSociais, categorias, tiposProduto, marcas, paginas, cupons, promocoes] = await Promise.all([
        ...
        this.buscarMarcas(),
        this.buscarPaginas(),
        this.buscarCupons(config),
        this.buscarPromocoes(config)
      ]);
  ...
  async buscarCupons(config) {
    if (config && Array.isArray(config.cupons) && config.cupons.length > 0) {
      return config.cupons.filter(c => c.ativo !== false);
    }
  ...
  async buscarPromocoes(config) {
    if (config && Array.isArray(config.promocoes) && config.promocoes.length > 0) {
      return config.promocoes.filter((p) => p.ativo !== false);
    }
    ...
    try {
      try {
        ...
        const promocoes = [];
        snapshot.forEach((doc) => {
          promocoes.push({ id: doc.id, ...doc.data() });
        });
  ...
  getConfigPadrao() {
    return {
      ...
      compartilhamentoTitulo: '',
      compartilhamentoDescricao: '',
      compartilhamentoImagemUrl: '',
      telefone: '',
      email: '',
      endereco: '',
      customizacao: {},
