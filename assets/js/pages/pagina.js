...
const PaginaRender = {
  ...
  toAbsoluteUrl(url) {
    const safe = DomUtils.sanitizeUrl(url || '');
    if (!safe) return '';
    try {
  ...
  setFavicon(url) {
    const safe = this.toAbsoluteUrl(url || '');
    if (!safe) return;
  ...
  updateShareMeta(config, pagina) {
    const loja = String(config.nomeLoja || 'Catalogo').trim();
    const baseTitle = `${loja} - Pagina`;
    const pageTitle = pagina?.titulo ? `${pagina.titulo} - ${loja}` : baseTitle;
    ...
    const fromPageText = pagina?.conteudoTexto
      ? String(pagina.conteudoTexto).trim()
      : (pagina?.conteudoHtml ? this.plainTextFromHtml(pagina.conteudoHtml) : '');

    const descricao = (fromPageText || fromConfig).slice(0, 220);
    ...
    this.setFavicon(config.faviconUrl || config.logoUrl || 'assets/img/LOGO SITE.png');

    this.setMeta('name', 'description', descricao);
    this.setMeta('property', 'og:type', 'website');
    this.setMeta('property', 'og:locale', 'pt_BR');
    this.setMeta('property', 'og:site_name', loja);
    this.setMeta('property', 'og:title', config.compartilhamentoTitulo || pageTitle);
    this.setMeta('property', 'og:description', descricao);
    this.setMeta('property', 'og:url', currentUrl);
    if (imageUrl) {
      this.setMeta('property', 'og:image', imageUrl);
      this.setMeta('property', 'og:image:alt', loja);
      this.setMeta('name', 'twitter:image', imageUrl);
    }
    this.setMeta('name', 'twitter:card', 'summary_large_image');
    this.setMeta('name', 'twitter:title', config.compartilhamentoTitulo || pageTitle);
    this.setMeta('name', 'twitter:description', descricao);
