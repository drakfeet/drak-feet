...
function updateShareMeta(config) {
  ...
  setFavicon(config.faviconUrl || config.logoUrl || 'assets/img/LOGO SITE.png');

  document.title = titulo;
  setMeta('name', 'description', descricao);
  setMeta('property', 'og:type', 'website');
  setMeta('property', 'og:locale', 'pt_BR');
  setMeta('property', 'og:site_name', loja);
  setMeta('property', 'og:title', config.compartilhamentoTitulo || titulo);
  setMeta('property', 'og:description', descricao);
  setMeta('property', 'og:url', currentUrl);
  if (imageUrl) {
    setMeta('property', 'og:image', imageUrl);
    setMeta('property', 'og:image:alt', loja);
    setMeta('name', 'twitter:image', imageUrl);
  }
  setMeta('name', 'twitter:card', 'summary_large_image');
  setMeta('name', 'twitter:title', config.compartilhamentoTitulo || titulo);
...
function atualizarCatalogo(produtos, config) {
  ...
  if (window.WishlistService) {
    WishlistService.syncWithCatalog(produtos);
  }
}
...
async function inicializarCatalogo() {
  try {
    ...
    if (window.WishlistService) WishlistService.setConfig(config);
    ...
    if (window.CartService) {
      ...
      CartService.setCouponOptions(config);
      CartService.setPromotionOptions(config);
    }
    ...
    if (btnWishlist) btnWishlist.addEventListener('click', abrirWishlist);
    if (wishlistClose) wishlistClose.addEventListener('click', fecharWishlist);
    if (wishlistOverlay) wishlistOverlay.addEventListener('click', fecharWishlist);
    ...
    const logoScrollTop = document.getElementById('logoScrollTop');
    if (logoScrollTop) {
