...
- `public/assets/js/pages/app.js`: Lógica principal do catálogo.
...
---
# 7) Segurança do Login

- O botão de correção automática de usuário foi removido do login para reduzir superfície de ataque.
- Correções de role/status devem ser feitas apenas por backend confiável (Admin SDK / Cloud Functions / script CLI).

---
# 8) Seed do banco via CLI (Node)

```bash
npm install
npm run seed:firestore -- --serviceAccount ./serviceAccountKey.json
```

Para sobrescrever documentos base existentes:

```bash
npm run seed:firestore -- --serviceAccount ./serviceAccountKey.json --overwrite
```

Comando único (seed + rules + indexes):

```bash
npm run setup:firebase -- --serviceAccount ./serviceAccountKey.json --projectId SEU_PROJECT_ID
```

Opcional (sobrescrever docs base):
