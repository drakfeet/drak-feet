/**
 * UI de Comunicação
 */

const ComunicacaoUI = {
  init() {
    setupEventListeners();
    carregarConfiguracoes();
  }
};

window.ComunicacaoUI = ComunicacaoUI;

function setupEventListeners() {
  document.getElementById('configComunicacaoForm')
    ?.addEventListener('submit', salvarConfiguracoes);

  document.getElementById('mensagemPadrao')
    ?.addEventListener('input', atualizarPreviewProduto);

  document.getElementById('mensagemCarrinho')
    ?.addEventListener('input', atualizarPreviewCarrinho);

  document.getElementById('mensagemCarrinhoItem')
    ?.addEventListener('input', atualizarPreviewCarrinho);

  document.getElementById('avisoAtivo')
    ?.addEventListener('change', atualizarEstadoAvisoGeral);

  document.addEventListener('click', (event) => {
    const tag = event.target.closest('.variavel-tag[data-target-field][data-variable]');
    if (!tag) return;
    inserirVariavel(tag.dataset.targetField, tag.dataset.variable);
  });

  document.getElementById('compartilhamentoImagemUpload')
    ?.addEventListener('change', uploadImagemCompartilhamento);

  document.getElementById('btnCopiarLinkCompartilhamento')
    ?.addEventListener('click', copiarLinkCompartilhamento);
}

async function carregarConfiguracoes() {
  const config = await ConfigService.buscar();

  setValue('whatsapp', config.whatsapp);
  setValue(
    'mensagemPadrao',
    config.mensagemPadrao ||
    'Olá! Gostaria de fazer um pedido:\n\n*Produto:* {produto}\n*Marca:* {marca}\n*Tamanho:* {tamanho}\n*Pagamento:* {pagamento}\n*Entrega:* {entrega}\n*Valor:* R$ {valor}'
  );

  setValue(
    'mensagemCarrinho',
    config.mensagemCarrinho ||
    'Olá! Gostaria de finalizar meu pedido:\n\n{produtos}\n\nResumo: {quantidade} item(ns) | Pagamento: {pagamento}\nSubtotal: R$ {subtotal} | Desconto: R$ {desconto}\nCupom: {cupom}\nEntrega: {entrega} | Frete: R$ {frete}\nTotal: R$ {total}'
  );
  setValue(
    'mensagemCarrinhoItem',
    config.mensagemCarrinhoItem ||
    '{numero}. *{produto}* - {marca}\n   Tamanho: {tamanho} | Pagamento: {pagamento}\n   Qtd: {quantidade} | Valor unit.: R$ {valor} | Subtotal: R$ {subtotal}'
  );

  setChecked('whatsappFlutuante', config.whatsappFlutuante !== false);
  setValue('whatsappMensagemFlutuante', config.whatsappMensagemFlutuante);
  setValue('whatsappMensagemInicial', config.whatsappMensagemInicial);

  setChecked('exibirHorario', config.exibirHorario === true);
  setChecked('permitirForaHorario', config.permitirForaHorario === true);
  setValue('horarioSegSexInicio', config.horarioSegSexInicio || '09:00');
  setValue('horarioSegSexFim', config.horarioSegSexFim || '18:00');
  setValue('horarioSabInicio', config.horarioSabInicio || '09:00');
  setValue('horarioSabFim', config.horarioSabFim || '13:00');
  setChecked('atendeDOM', config.atendeDOM === true);
  setValue('mensagemForaHorario', config.mensagemForaHorario);

  setValue('telefone', config.telefone);
  setValue('email', config.email);
  setValue('endereco', config.endereco);
  setChecked('avisoAtivo', config.avisoAtivo !== false);
  setValue('avisoTexto', config.avisoTexto);
  setValue('avisoBotaoTexto', config.avisoBotaoTexto);
  setValue('avisoBotaoUrl', config.avisoBotaoUrl);
  setValue('compartilhamentoTitulo', config.compartilhamentoTitulo);
  setValue('compartilhamentoDescricao', config.compartilhamentoDescricao);
  setValue('compartilhamentoImagemUrl', config.compartilhamentoImagemUrl);
  renderPreviewImagemCompartilhamento(config.compartilhamentoImagemUrl);
  atualizarLinkCompartilhamento(config);
  atualizarEstadoAvisoGeral();

  atualizarPreviewProduto();
  atualizarPreviewCarrinho();
}

function atualizarPreviewProduto() {
  const mensagem = document.getElementById('mensagemPadrao')?.value || '';
  const preview = document.getElementById('previewMensagemProduto');
  if (!preview) return;

  const texto = mensagem
    .replace(/{produto}/g, 'Produto Exemplo Premium')
    .replace(/{marca}/g, 'Marca Exemplo')
    .replace(/{tamanho}/g, 'Único')
    .replace(/{pagamento}/g, 'PIX')
    .replace(/{entrega}/g, 'Retirar na Loja')
    .replace(/{valor}/g, '299,90');

  DomUtils.clear(preview);
  const pre = document.createElement('pre');
  pre.textContent = texto;
  preview.appendChild(pre);
}

function atualizarPreviewCarrinho() {
  const mensagem = document.getElementById('mensagemCarrinho')?.value || '';
  const itemTemplate = document.getElementById('mensagemCarrinhoItem')?.value
    || '{numero}. *{produto}* - {marca}\n   Tamanho: {tamanho} | Pagamento: {pagamento}\n   Qtd: {quantidade} | Valor unit.: R$ {valor} | Subtotal: R$ {subtotal}';
  const preview = document.getElementById('previewMensagemCarrinho');
  if (!preview) return;

  const item1 = itemTemplate
    .replace(/{numero}/g, '1')
    .replace(/{produto}/g, 'Produto Exemplo Premium')
    .replace(/{marca}/g, 'Marca X')
    .replace(/{tamanho}/g, 'Único')
    .replace(/{pagamento}/g, 'PIX')
    .replace(/{quantidade}/g, '1')
    .replace(/{valor}/g, '299,90')
    .replace(/{subtotal}/g, '299,90');

  const item2 = itemTemplate
    .replace(/{numero}/g, '2')
    .replace(/{produto}/g, 'Camisa Adidas')
    .replace(/{marca}/g, 'Adidas')
    .replace(/{tamanho}/g, 'M')
    .replace(/{pagamento}/g, 'CARTAO')
    .replace(/{quantidade}/g, '2')
    .replace(/{valor}/g, '149,95')
    .replace(/{subtotal}/g, '299,90');

  const produtosExemplo = `${item1}\n\n${item2}`;

  const exemplo = mensagem
    .replace(/{produtos}/g, produtosExemplo)
    .replace(/{itens}/g, 'Produto Exemplo Premium x1\nCamisa Adidas x2')
    .replace(/{item}/g, produtosExemplo)
    .replace(/{quantidade}/g, '3')
    .replace(/{subtotal}/g, '609,80')
    .replace(/{desconto}/g, '20,00')
    .replace(/{cupom}/g, 'PRIMEIRA10')
    .replace(/{total}/g, '599,80')
    .replace(/{total_pix}/g, '299,90')
    .replace(/{total_cartao}/g, '299,90')
    .replace(/{pagamento}/g, 'Variado')
    .replace(/{entrega}/g, 'Entrega via Motoboy')
    .replace(/{frete}/g, '10,00')
    .replace(/{observacoes}/g, 'Entregar na portaria');

  DomUtils.clear(preview);
  const pre = document.createElement('pre');
  pre.textContent = exemplo;
  preview.appendChild(pre);
}

function inserirVariavel(campoId, variavel) {
  const campo = document.getElementById(campoId);
  if (!campo) return;

  const start = campo.selectionStart ?? campo.value.length;
  const end = campo.selectionEnd ?? campo.value.length;

  campo.value =
    campo.value.substring(0, start) +
    variavel +
    campo.value.substring(end);

  campo.focus();
  campo.setSelectionRange(start + variavel.length, start + variavel.length);

  if (campoId === 'mensagemPadrao') {
    atualizarPreviewProduto();
  } else {
    atualizarPreviewCarrinho();
  }
}

async function salvarConfiguracoes(e) {
  e.preventDefault();

  const whatsapp = document.getElementById('whatsapp')?.value.trim();
  if (!CryptoService.validatePhone(whatsapp)) {
    alert('Número de WhatsApp inválido');
    return;
  }

  const config = {
    whatsapp: DomUtils.sanitizeText(whatsapp, 15),
    mensagemPadrao: DomUtils.sanitizeText(getValue('mensagemPadrao'), 1200),
    mensagemCarrinho: DomUtils.sanitizeText(getValue('mensagemCarrinho'), 2000),
    mensagemCarrinhoItem: DomUtils.sanitizeText(getValue('mensagemCarrinhoItem'), 1500),

    whatsappFlutuante: isChecked('whatsappFlutuante'),
    whatsappMensagemFlutuante: DomUtils.sanitizeText(getValue('whatsappMensagemFlutuante'), 120),
    whatsappMensagemInicial: DomUtils.sanitizeText(getValue('whatsappMensagemInicial'), 400),

    exibirHorario: isChecked('exibirHorario'),
    permitirForaHorario: isChecked('permitirForaHorario'),
    horarioSegSexInicio: getValue('horarioSegSexInicio'),
    horarioSegSexFim: getValue('horarioSegSexFim'),
    horarioSabInicio: getValue('horarioSabInicio'),
    horarioSabFim: getValue('horarioSabFim'),
    atendeDOM: isChecked('atendeDOM'),
    mensagemForaHorario: DomUtils.sanitizeText(getValue('mensagemForaHorario'), 400),

    telefone: DomUtils.sanitizeText(getValue('telefone'), 30),
    email: DomUtils.sanitizeText(getValue('email'), 254),
    endereco: DomUtils.sanitizeText(getValue('endereco'), 300),
    avisoAtivo: isChecked('avisoAtivo'),
    avisoTexto: DomUtils.sanitizeText(getValue('avisoTexto'), 500),
    avisoBotaoTexto: DomUtils.sanitizeText(getValue('avisoBotaoTexto'), 120),
    avisoBotaoUrl: DomUtils.sanitizeUrl(getValue('avisoBotaoUrl')),
    compartilhamentoTitulo: DomUtils.sanitizeText(getValue('compartilhamentoTitulo'), 120),
    compartilhamentoDescricao: DomUtils.sanitizeText(getValue('compartilhamentoDescricao'), 220),
    compartilhamentoImagemUrl: DomUtils.sanitizeUrl(getValue('compartilhamentoImagemUrl'))
  };

  const result = await ConfigService.salvar(config);
  alert(result.success ? 'Salvo com sucesso!' : 'Erro ao salvar');
}

function setValue(id, value = '') {
  const el = document.getElementById(id);
  if (el) el.value = value;
}

function setChecked(id, value = false) {
  const el = document.getElementById(id);
  if (el) el.checked = value;
}

function getValue(id) {
  return document.getElementById(id)?.value.trim() || '';
}

function isChecked(id) {
  return document.getElementById(id)?.checked || false;
}

function atualizarEstadoAvisoGeral() {
  const avisoAtivo = isChecked('avisoAtivo');
  ['avisoTexto', 'avisoBotaoTexto', 'avisoBotaoUrl'].forEach((id) => {
    const input = document.getElementById(id);
    if (!input) return;
    input.disabled = !avisoAtivo;
  });
}

async function uploadImagemCompartilhamento(e) {
  const file = e.target?.files?.[0];
  if (!file) return;

  if (!file.type.startsWith('image/')) {
    alert('Selecione apenas imagem (JPG/PNG/WebP).');
    e.target.value = '';
    return;
  }

  if (file.size > 3 * 1024 * 1024) {
    alert('Imagem muito grande. Limite: 3MB.');
    e.target.value = '';
    return;
  }

  const preview = document.getElementById('compartilhamentoImagemPreview');
  if (preview) {
    preview.innerHTML = '<small>Enviando imagem...</small>';
  }

  try {
    const imageUrl = await uploadImagemCloudinary(file);
    setValue('compartilhamentoImagemUrl', imageUrl);
    renderPreviewImagemCompartilhamento(imageUrl);
  } catch (error) {
    console.error('Erro no upload da imagem de compartilhamento:', error);
    alert('Falha ao enviar imagem. Tente novamente.');
    renderPreviewImagemCompartilhamento(getValue('compartilhamentoImagemUrl'));
  } finally {
    e.target.value = '';
  }
}

async function uploadImagemCloudinary(file) {
  const cloudName = String(AppConfig?.cloudinary?.cloudName || '').trim();
  const uploadPreset = String(AppConfig?.cloudinary?.uploadPreset || '').trim();
  if (!cloudName || !uploadPreset) {
    throw new Error('Cloudinary não configurado em AppConfig.');
  }

  const formData = new FormData();
  formData.append('file', file);
  formData.append('upload_preset', uploadPreset);

  const response = await fetch(`https://api.cloudinary.com/v1_1/${cloudName}/image/upload`, {
    method: 'POST',
    body: formData
  });
  const data = await response.json();
  if (!response.ok || !data?.secure_url) {
    throw new Error(data?.error?.message || 'Upload não retornou URL.');
  }
  return data.secure_url;
}

function renderPreviewImagemCompartilhamento(url) {
  const preview = document.getElementById('compartilhamentoImagemPreview');
  if (!preview) return;

  const safeUrl = DomUtils.sanitizeUrl(url || '');
  if (!safeUrl) {
    preview.innerHTML = '';
    return;
  }

  preview.innerHTML = `
    <img src="${safeUrl}" alt="Imagem de compartilhamento" style="max-width: 280px; border-radius: 8px; display:block; margin-bottom: 8px;">
    <small>Imagem atual pronta para compartilhamento.</small>
  `;
}

function atualizarLinkCompartilhamento(config = {}) {
  const input = document.getElementById('linkCompartilhamento');
  if (!input) return;

  const rawCatalogoUrl = DomUtils.sanitizeUrl(config.catalogoUrl || '');
  let base = window.location.origin;

  if (rawCatalogoUrl) {
    try {
      base = new URL(rawCatalogoUrl).origin;
    } catch (error) {
      base = window.location.origin;
    }
  }

  input.value = `${base}/share`;
}

async function copiarLinkCompartilhamento() {
  const input = document.getElementById('linkCompartilhamento');
  if (!input || !input.value) {
    alert('Link de compartilhamento não disponível.');
    return;
  }

  const text = input.value.trim();
  try {
    if (navigator.clipboard && navigator.clipboard.writeText) {
      await navigator.clipboard.writeText(text);
    } else {
      input.focus();
      input.select();
      document.execCommand('copy');
    }
    alert('Link de compartilhamento copiado!');
  } catch (error) {
    alert('Não foi possível copiar automaticamente. Copie manualmente o link exibido.');
  }
}

window.inserirVariavel = inserirVariavel;


