...
const ProdutosUI = {
  ...
  async carregarFormulario(produtoId) {
    try {
      ...
      this.originalImageUrl = produto.imagemUrl || '';
  ...
  async processarUpload(e) {
    ...
    this.mostrarMensagem('Imagem selecionada. O upload será feito ao salvar.', 'info');
  },
  ...
  async salvarProduto(e) {
    ...
    let imagemUrlFinal = String(form.imagemUrlHidden.value || '').trim();

    if (this.selectedImageFile) {
      this.isUploading = true;
      this.mostrarLoading('Enviando imagem...');
      try {
        imagemUrlFinal = await this.uploadComProgresso(this.selectedImageFile);
        form.imagemUrlHidden.value = imagemUrlFinal;
        this.originalImageUrl = imagemUrlFinal;
        this.selectedImageFile = null;
        this.imageDirty = false;
      } catch (uploadError) {
    ...
    if (!imagemUrlFinal && this.originalImageUrl) {
      imagemUrlFinal = this.originalImageUrl;
      form.imagemUrlHidden.value = imagemUrlFinal;
    }
