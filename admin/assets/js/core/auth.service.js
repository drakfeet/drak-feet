...
const AuthService = {
  ...
  async login(email, password) {
    ...
    try {
      if (this.isAccountLocked(emailKey)) {
        const remainingTime = this.getRemainingLockTime(emailKey);
      ...
      if (window.RateLimiter && !RateLimiter.allowBucket(`login:${emailKey}`, 5, 1 / 120)) {
      ...
      const healthcheck = await this.runProfileHealthcheck(result.user);
      if (!healthcheck.ok) {
        await firebase.auth().signOut();
        return {
          success: false,
          error: healthcheck.error || 'Perfil de usuário inválido em usuarios/{uid}.'
        };
      }
      ...
      const shouldCountFailedAttempt = [
        'auth/user-not-found',
        'auth/wrong-password',
        'auth/invalid-email',
      ...
      switch (error.code) {
        ...
        case 'permission-denied':
          message = 'Acesso negado no Firestore para carregar seu perfil. Verifique o documento usuarios/{uid}.';
          break;
        default:
  ...
  async runProfileHealthcheck(user) {
    if (!user || !user.uid) {
      return { ok: false, error: 'Usuário não autenticado' };
    }

    try {
      const ref = firebaseDb.collection('usuarios').doc(user.uid);
      let doc = await ref.get();

      if (!doc.exists) {
        if (window.UsersService && typeof UsersService.ensureProfile === 'function') {
        ...
        if (!doc.exists) {
          return {
            ok: false,
            error: 'Perfil não provisionado em usuarios/{uid}. Solicite liberação ao administrador.'
          };
        }
      }
      ...
      if (!role || !['admin', 'editor', 'analista', 'analyst'].includes(role)) {
        return {
          ok: false,
          error: 'Perfil sem role válida em usuarios/{uid}. Defina role como admin, editor ou analista.'
        };
      }
      ...
      return {
        ok: false,
        error: `Falha ao validar usuarios/{uid}: ${error?.message || error}`
      };
    }
  },
  ...
  registerFailedAttempt(email) {
    ...
    this.loginAttempts[key].lastAttempt = Date.now();
