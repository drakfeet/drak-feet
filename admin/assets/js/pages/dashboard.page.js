...
const DashboardPage = {
    ...
    document.getElementById('bootstrapFirestoreBtn')?.addEventListener('click', async (e) => {
      ...
      try {
        ...
        alert(
          ...
          `paginas: ${s.paginas || 0}\n` +
          `cupons: ${s.cupons || 0}` +
          '\\n\\nPara publicar rules + indices em 1 passo, rode no terminal:\\n' +
  ...
  async carregarDashboard() {
    try {
      ...
      const funil = DashboardService.funilConversao(dados.metricas, 30);
      this.renderizarGraficoFunil(funil);

      const tiposEvento = DashboardService.agruparPorTipoEvento(dados.metricas, 30);
      this.renderizarGraficoTiposEvento(tiposEvento);
  ...
  renderizarGraficoFunil(funil) {
    const ctx = document.getElementById('graficoFunil');
    if (!ctx) return;

    if (this.graficoFunil) this.graficoFunil.destroy();

    this.graficoFunil = new Chart(ctx, {
      type: 'bar',
      data: {
        labels: funil.map((f) => f.etapa),
        datasets: [{
          label: 'Eventos',
          data: funil.map((f) => f.total),
  ...
  renderizarGraficoTiposEvento(tipos) {
    const ctx = document.getElementById('graficoTiposEvento');
    if (!ctx) return;

    if (this.graficoTiposEvento) this.graficoTiposEvento.destroy();

    this.graficoTiposEvento = new Chart(ctx, {
      type: 'doughnut',
      data: {
  ...
  renderizarTopProdutos(produtos) {
    ...
    produtos.forEach((produto, index) => {
      ...
      num.textContent = `${index + 1}${String.fromCharCode(186)}`;
  ...
  async renderizarHealthPanel() {
    ...
    let cloudinaryStatus = 'Não configurado';
    try {
      ...
      if (cloudName && uploadPreset) {
        cloudinaryStatus = 'Configurado';
      } else if (cloudName || uploadPreset) {
        cloudinaryStatus = 'Parcial';
      }
    } catch (e) {
