...
const MetricasPage = {
  ...
  bindAcoesAdmin() {
    const limparBtn = document.getElementById('btnLimparMetricas');
    if (!limparBtn) return;

    limparBtn.addEventListener('click', async () => {
      if (!window.MetricasService) return;

      const confirmou = window.confirm(
        'Isso vai apagar todas as métricas registradas.\nOs produtos NÃO serão apagados.\n\nDeseja continuar?'
      );
      if (!confirmou) return;

      const confirmouFinal = window.confirm(
        'Confirmação final: limpar TODAS as métricas agora?'
      );
      if (!confirmouFinal) return;

      const textoOriginal = limparBtn.textContent;
      limparBtn.disabled = true;
      limparBtn.textContent = 'Limpando...';

      try {
        const result = await MetricasService.limparTodasMetricas();
        if (!result || !result.success) {
          alert(result?.error || 'Não foi possível limpar as métricas.');
          return;
        }

        alert(`Métricas limpas com sucesso. Registros removidos: ${result.deletedCount}`);
        this.currentPage = 1;
        this.pageAnchors = { 1: null };
        await this.carregarMetricas();
      } catch (error) {
        alert('Erro ao limpar métricas. Tente novamente.');
      } finally {
        limparBtn.disabled = false;
        limparBtn.textContent = textoOriginal;
      }
    });
  },
