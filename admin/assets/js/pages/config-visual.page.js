...
const ConfigVisualPage = {
  ...
  bindEvents() {
    ...
    document.getElementById('faviconUpload')?.addEventListener('change', (e) => this.onFaviconUpload(e));
  ...
  async onFaviconUpload(e) {
    ...
    if (file.size > 1024 * 1024) {
      alert('Imagem muito grande (max. 1MB)');
      return;
    }

    try {
      const loadingEl = document.getElementById('faviconPreview');
      DomUtils.clear(loadingEl);
      const p = document.createElement('p');
      p.textContent = 'Enviando favicon...';
      loadingEl.appendChild(p);
      loadingEl.style.display = 'block';
  ...
  async carregarConfiguracoes() {
    ...
    if (config.faviconUrl) {
      ...
      DomUtils.clear(faviconPreview);
      const favImg = document.createElement('img');
      favImg.src = config.faviconUrl;
      favImg.alt = 'Favicon';
      favImg.style.maxWidth = '64px';
      favImg.style.maxHeight = '64px';
      faviconPreview.appendChild(favImg);
