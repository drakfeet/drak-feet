...
const DashboardService = {
  ...
  /**
   * Distribuição por tipo de evento (janela de dias)
   */
  agruparPorTipoEvento(metricas, dias = 30) {
    const inicio = new Date(Date.now() - dias * 86400000);
    const validas = metricas.filter(
      m => m.timestamp instanceof Date && m.timestamp >= inicio
    );

    const labels = {
      page_view: 'Visualização',
      product_view: 'Produto',
      whatsapp_click: 'WhatsApp Produto',
  ...
  /**
   * Funil básico de conversão (janela de dias)
   */
  funilConversao(metricas, dias = 30) {
    const inicio = new Date(Date.now() - dias * 86400000);
    const validas = metricas.filter(
      m => m.timestamp instanceof Date && m.timestamp >= inicio
    );

    const contar = (tipo) => validas.filter((m) => m.tipo === tipo).length;

    return [
      { etapa: 'Visualizações', total: contar('page_view') },
      { etapa: 'Cliques WhatsApp', total: contar('whatsapp_click') },
      { etapa: 'Add ao Carrinho', total: contar('add_to_cart') },
      { etapa: 'Finalizações', total: contar('checkout_whatsapp') }
    ];
  },
